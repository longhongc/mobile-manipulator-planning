...
bool Problem::check_collision(
    State state1, State state2, int points) {

  auto intermediate_states = interpolate(state1, state2, points);
  ...
  for (int i=0; i < points; ++i) {
    auto& state = intermediate_states[i];
    if (this->check_collision(state.base_pose, robot->workspace_radius)) {
